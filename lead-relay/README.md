...
1. Chọn hàm `setupHeader` → **Run** (lần đầu Google hỏi cấp quyền → Allow).
   Sheet sẽ có dòng tiêu đề 12 cột.
2. Chọn `testAppend` → **Run** → sheet có thêm 1 dòng test. Xoá dòng test sau khi xong.
3. **Deploy → New deployment → Web app**
   - Execute as: **Me**
   - Who has access: **Anyone**
   - → copy **Web app URL** (dạng `https://script.google.com/macros/s/AKfy…/exec`)
4. Mở URL đó trên trình duyệt: thấy `{"ok":true,...}` là xong.
...
dán URL ở bước 4 vào. Xong — submit form là sheet có dòng mới.
